<xsl:stylesheet version="2.0" 
  ...
  <!-- div !-->
  <xsl:template match="tei:div[@type='adhyāya']">
    <xsl:apply-templates/>
    <xsl:text>======================================================
</xsl:text>
  </xsl:template>
  <xsl:template match="tei:div[@type='pāda']">
    <xsl:apply-templates/>
    <xsl:text>-------------------------------------------------------
</xsl:text>
  </xsl:template>
  ...
  <xsl:template match="tei:s">
    <xsl:apply-templates/>
    <xsl:choose>
      <xsl:when test="$language = 'san-Latn'">
	<xsl:text>. </xsl:text>
      </xsl:when>
      <xsl:otherwise>
	<xsl:text> । </xsl:text>
  ...
  <xsl:template match="tei:quote[@type='sūtra']">
	...
	<xsl:apply-templates/>
	<xsl:text> ॥ </xsl:text>
	<xsl:value-of select="translate(./ancestor-or-self::tei:div[@type='adhyāya'][1]/@n,'1234567890','१२३४५६७८९०')"/>
	<xsl:text>।</xsl:text>
	<xsl:value-of select="translate(./ancestor-or-self::tei:div[@type='pāda'][1]/@n,'1234567890','१२३४५६७८९०')"/>
	<xsl:text>।</xsl:text>
	<xsl:value-of select="translate(@n,'1234567890','१२३४५६७८९०')"/>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:text>
</xsl:text>
